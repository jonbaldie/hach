...
module Agent.PermissionsSpec (spec) where
...
spec :: Spec
spec = describe "Agent.Permissions" $ do
  describe "Default mode" $ do
    it "allows read-only tools" $ do
      let args = object ["path" .= ("src/Agent.hs" :: String)]
    ...
    it "asks for file write tools" $ do
      let args = object ["path" .= ("src/Agent.hs" :: String), "content" .= ("" :: String)]
  ...
  describe "AcceptEdits mode" $ do
    it "auto-approves file writes and edits" $ do
      let writeArgs = object ["path" .= ("src/Agent.hs" :: String), "content" .= ("" :: String)]
          editArgs = object ["path" .= ("src/Agent.hs" :: String), "old_content" .= ("a" :: String), "new_content" .= ("b" :: String)]
